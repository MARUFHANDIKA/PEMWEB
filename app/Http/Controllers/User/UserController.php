...
class UserController extends Controller
{
    ...
    public function home() {
        ...
        return view('user.main.home', compact('menu', 'categories', 'carts', 'orders'));
    }

    public function carts() {
        $cartList = Cart::with('product')->where('user_id', Auth::id())->get();
$totalPrice = 0;
...
return view('user.cart.carts', compact('cartList', 'totalPrice'));
    ...
    public function filter($id) {
        $menu = Product::where('category_id', $id)->orderBy('created_at', 'desc')->get();
    ...
    private function getTotalPrice($cartList) {
        ...
        foreach($cartList as $cart) {
            if (!$cart->product) {
                logger("Cart ID {$cart->id} tidk memiliki relasi produk.");
                continue;
            }
            $totalPrice += $cart->product->price * $cart->quantity;
        }
