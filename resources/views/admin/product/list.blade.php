...
@section('content')
    <main class="container py-5">
        <div class="mb-4 d-flex justify-content-between align-items-center">
            <h2 class="fw-semibold">List product-?stock</h2>
            <div>
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-6">
                <form action="{{ route('product#list') }}" method="GET" class="d-flex justify-content-end">
                    <input type="text" name="key" class="form-control w-50" placeholder="Search..."
                        value="{{ request('key') }}">
                        ...
                        @foreach ($menu as $menus)
                            <tr>
                                <td style="width: 100px">
                                    <img src="{{ asset('storage/' . $menus->image) }}" alt="{{ $menus->name }}"
                                        class="img-thumbnail">
                                </td>
                                <td>{{ $menus->name }}</td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ route('product#show', $menus->id) }}" class="btn btn-sm btn-outline-primary"
                                            title="View">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a href="{{ route('product#edit', $menus->id) }}" class="btn btn-sm btn-outline-warning"
                                            title="Edit">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <a href="{{ route('product#delete', $menus->id) }}" class="btn btn-sm btn-outline-danger"
                                            title="Delete">
